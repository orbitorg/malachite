...
use libp2p::{floodsub, gossipsub, identify, SwarmBuilder};
use tokio::sync::mpsc;
...
async fn handle_swarm_event(
    ...
    match event {
        ...
        SwarmEvent::Behaviour(NetworkEvent::FloodSub(event)) => {
            return handle_floodsub_event(event, metrics, swarm, state, tx_event).await;
        }
...
async fn handle_floodsub_event(
    ...
    match dbg!(event) {
        ...
        FloodsubEvent::Message(message) => {
            ...
            let Some(topic) = message.topics.first() else {
                trace!("Received message from {peer_id} without any channel",);
                return ControlFlow::Continue(());
            };

            let Some(channel) = Channel::from_floodsub_topic(topic) else {
            ...
            let event = Event::Message(channel, peer_id, message.data);
